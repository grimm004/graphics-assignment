...
class CGCoursework extends Application {
    ...
    async initialise() {
        ...
        this.sceneGraph = new LitSceneNode(floorMesh, Vector3.zeros, Vector3.zeros, Vector3.ones, [...shelves, this.table])
    ...
    keyDown(key) {
        if (key === "r") {
            CGCoursework.setSliderVector("PosSlider", 0.0);
            CGCoursework.setSliderVector("RotSlider", 0.0, "yaw", "pitch", "roll");
            CGCoursework.setSliderValue("scaleSlider", 1.0);
            CGCoursework.setSliderValue("tableRotationSlider", 0.0);
    ...
    update(deltaTime) {
        ...
        CGCoursework.setInnerText("fpsLabel", this.frameCounter.averageFrameRate.toFixed(2));

        // Set table orientation
        this.table.orientation = new Vector3(Math.radians(CGCoursework.getSliderValue("tableRotationSlider")), 0.0, 0.0);
        ...
        const worldTransform = Matrix4.positionOrientationScale(
            CGCoursework.getSliderVector("PosSlider"),
            CGCoursework.getSliderVector("RotSlider", "yaw", "pitch", "roll").apply(x => Math.radians(x)),
            new Vector3(CGCoursework.getSliderValue("scaleSlider"))
        );
    ...
    static getSliderVector(idSuffix, xPrefix = "x", yPrefix = "y", zPrefix = "z", scale = 10.0) {
        return new Vector3(
            CGCoursework.getSliderValue(`${xPrefix}${idSuffix}`, scale),
            CGCoursework.getSliderValue(`${yPrefix}${idSuffix}`, scale),
            CGCoursework.getSliderValue(`${zPrefix}${idSuffix}`, scale)
        );
    }
